...
def _default_user_agent() -> str:
    ...
    return f"prime-mcp-server python/{python_version}"
    ...
    async def request(
        ...
        try:
            ...
            if e.response.status_code == 401:
                raise APIError("API key unauthorized. Check PRIME_API_KEY.") from e
            if e.response.status_code == 402:
                raise APIError("Payment required. Check billing status.") from e
