...
def test_tunnel_init_with_http_auth():
    """Test Tunnel initialization with HTTP basic auth credentials."""
    tunnel = Tunnel(local_port=9000, http_user="alice", http_password="s3cret")
    assert tunnel.http_user == "alice"
    assert tunnel.http_password == "s3cret"
...
async def test_client_create_tunnel_sends_http_auth(monkeypatch):
    ...
    tunnel_info = await client.create_tunnel(
        local_port=8765, http_user="alice", http_password="s3cret"
    )

    assert captured["json"]["http_user"] == "alice"
    assert captured["json"]["http_password"] == "s3cret"
