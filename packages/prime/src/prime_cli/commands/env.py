...
def _init_sandbox_environment(name: str, path: str) -> Path:
...
def _init_ts_environment(name: str, path: str) -> Path:
    env_id_underscore = name.replace("-", "_")
    local_dir = Path(path) / env_id_underscore
    local_dir.mkdir(parents=True, exist_ok=True)
...
def init(
    ...
    try:
        ...
        if sandbox:
            created_path = _init_sandbox_environment(name, path)
            ...
            created_path = _init_ts_environment(name, path)
