...
def start_tunnel(
    ...
    team_id: Optional[str] = typer.Option(
        None, "--team-id", help="Team ID for team tunnel (uses config team_id if not specified)"
    ),
) -> None:
...
def list_tunnels(
    team_id: Optional[str] = typer.Option(
        None, "--team-id", help="Filter by team ID (uses config team_id if not specified)"
    ),
) -> None:
    """List active tunnels."""
