...
from .platform_preview import PlatformPreviewResult, preview_lab_config
...
class ConfigRunScreen(Screen[None]):
    ...
    def __init__(self, item: LabItem) -> None:
        ...
        self._platform_preview: PlatformPreviewResult | None = None
    ...
    async def _on_key(self, event: events.Key) -> None:
        if event.key != "b" or isinstance(self.focused, Input):
            return
    ...
    @on(Button.Pressed, "#config-platform-preview")
    def _platform_preview_pressed(self, _event: Button.Pressed) -> None:
        build = self._current_build()
        if build.errors:
            self._platform_preview = PlatformPreviewResult(
                status="error",
                message="Fix local validation errors before platform preview.",
            )
        else:
            self._platform_preview = preview_lab_config(self._config_kind, build.parsed)
        self._render_preview()
    ...
    def _preview_renderable(self) -> Group:
        return _preview_panel(
            ...
            self._clone_path,
            saved=self._saved,
            platform_preview=self._platform_preview,
        )
...
class ConfigLaunchScreen(Screen[None]):
    ...
    def on_mount(self) -> None:
        ...
        self._runner = ConfigLaunchRunner(
            ...
            append_output=lambda text: self._call_from_launch_thread(self._append_output, text),
...
def _field_widgets(values: dict[str, str]) -> list[Any]:
    fields = [
        ...
        ("config-batch-size", "Rollouts per batch", values.get("batch_size", ""), "integer"),
...
def _form_config(config: dict[str, Any], config_kind: str, field_value: Any) -> ConfigBuildResult:
    ...
    _set_reasoning_fields(
        updated,
        config_kind,
        field_value("config-enable-thinking"),
        field_value("config-reasoning-effort"),
        errors,
    )
    ...
    _validate_required_fields(updated, config_kind, errors)
    ...
    if validation.error is not None:
        ...
        _validate_config_schema(validation.parsed, config_kind, errors)
...
def _set_reasoning_fields(
    ...
    sampling.pop("reasoning_effort", None)
    parsed_enable_thinking = _parse_optional_bool("enable thinking", enable_thinking, errors)
    if parsed_enable_thinking is not None:
        sampling["enable_thinking"] = parsed_enable_thinking
    if reasoning_effort:
        if reasoning_effort not in {"low", "medium", "high"}:
            errors.append("reasoning effort must be low, medium, or high")
        else:
            sampling["reasoning_effort"] = reasoning_effort
    if "enable_thinking" in sampling and "reasoning_effort" in sampling:
        errors.append("enable thinking and reasoning effort cannot both be set")
    if not sampling:
...
def _initial_field_values(
    ...
    *,
    ...
    return {
        ...
        "enable_thinking": _field_text(sampling, "enable_thinking"),
        "reasoning_effort": _field_text(sampling, "reasoning_effort"),
    }
...
def _environment_tokens(config: dict[str, Any], config_kind: str) -> list[str]:
    ...
    token_key = "id"
    if config_kind == "rl" and not isinstance(raw_envs, list):
        return []
...
def _validate_required_fields(
    ...
    invalid_env_ids = [
        str(env.get("id") or "")
        for env in envs
        if isinstance(env, dict) and "/" not in str(env.get("id") or "")
    ]
    if invalid_env_ids:
        errors.append("environment id must use owner/name")
...
def _preview_panel(
    ...
    *,
    saved: bool = False,
    platform_preview: PlatformPreviewResult | None = None,
) -> Group:
    table = Table.grid(padding=(0, 2))
    table.add_column(style="bold dim", no_wrap=True)
    table.add_column()
    ...
    table.add_row("Platform preview", _platform_preview_text(platform_preview))
...
def _platform_preview_text(preview: PlatformPreviewResult | None) -> Text:
    if preview is None:
    ...
    text = Text(preview.message, style=style)
    for warning in preview.warnings:
        text.append(f"\n{warning}", style=STATUS_WARNING)
    return text
...
def _validate_config_schema(
    ...
    try:
        ...
        errors.extend(_schema_error_messages(exc))


def _schema_error_messages(exc: PydanticValidationError) -> list[str]:
    messages: list[str] = []
...
def _launch_command(config_kind: str, rel_path: str) -> str:
    ...
    if config_kind == "eval":
        return f"prime eval submit {quoted_path}"
