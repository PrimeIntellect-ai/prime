...
class TestPushSingleEval:
    ...
    def test_push_supports_native_v1_run(self, tmp_path, monkeypatch):
        """A native verifiers run (config.toml, no metadata.json) is pushable directly."""
        ...
        assert eval_id == "eval-123"
        assert captured["name"] == "gsm8k-v1-gpt-4"
        assert captured["model_name"] == "gpt-4"
        assert captured["metrics"] == {"reward": 1.0}
        assert captured["samples"] == [{"id": 1, "reward": 1.0, "example_id": 1}]
