...
@pytest.mark.parametrize(
    ("global_field", "global_value", "eval_field", "eval_value"),
    [
        ("backend", "anthropic_messages", "api_client_type", "openai_chat_completions"),
        ("api_client_type", "anthropic_messages", "backend", "openai_chat_completions"),
    ],
)
def test_hosted_eval_config_allows_backend_alias_override_across_scopes(
    tmp_path, global_field, global_value, eval_field, eval_value
):
    config_path = tmp_path / "eval.toml"
    config_path.write_text(
        f"""
...
[[eval]]
env_id = "gsm8k"
{eval_field} = "{eval_value}"
""".strip()
    )

    loaded = _load_hosted_eval_configs(str(config_path))[0]

    assert loaded["api_client_type"] == eval_value
    assert "backend" not in loaded
