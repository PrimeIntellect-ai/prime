    ...
    def mock_get(
        ...
        if endpoint == "/secrets/" or endpoint == "/secrets":
            ...
            for s in sample_secrets:
                stored_secret_id = s["id"]
                if isinstance(stored_secret_id, str) and (
                    stored_secret_id == secret_id or stored_secret_id.startswith(secret_id)
                ):
                    return {"data": s}
