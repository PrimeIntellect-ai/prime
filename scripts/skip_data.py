...
def skip_data(config: Config):
    ...
    assert config.train.batch_size % world_info.local_world_size == 0
    batch_size = config.train.batch_size // world_info.local_world_size

    assert batch_size % config.hardware.micro_batch_size == 0
    gradient_accumulation_steps = batch_size // config.hardware.micro_batch_size
    ...
    logger.info("starting skipping data up to step: %d", config.train.total_steps)
    ...
    logger.info("skipped data up to step: %d", config.train.total_steps)
