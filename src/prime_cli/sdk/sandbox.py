...
class Sandbox:
        ...
        while time.time() - start_time < max_wait_seconds:
            try:
                ...
                if sandbox.status == SandboxStatus.RUNNING:
                    ...
                    raise SandboxError(f"Sandbox failed to start with error status")
                elif sandbox.status == SandboxStatus.TERMINATED:
                    raise SandboxError(f"Sandbox was terminated during startup")
    ...
    def run(self, command: str, working_dir: Optional[str] = None, env: Optional[Dict[str, str]] = None) -> Result:
    ...
    def run_script(self, script_content: str, filename: str = "script.sh", working_dir: Optional[str] = None) -> Result:
